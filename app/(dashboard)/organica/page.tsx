...
const ENTIDADES_CERT = ['USDA Organic','UE Ecológico','BCS','CERES','IMO','Rainforest Alliance','otro']
const TIPOS_CERT = ['organico','transicion','fair_trade','rainforest']
    ...
    <div style={{ padding:'28px 32px', maxWidth:'1200px' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'20px' }}>
        ...
        <div style={{ display:'flex', gap:'8px' }}>
          {tab === 'parcelas' && <button onClick={() => setParcelaModal(true)} style={{ fontSize:'12px', padding:'8px 16px', background:'#166534', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontWeight:'500' }}>+ Parcela</button>}
          {tab === 'certificaciones' && <button onClick={() => { if(parcelas.length){ setSelParcela(parcelas[0]); setCertModal(true) } else toastError('Crea una parcela primero') }} style={{ fontSize:'12px', padding:'8px 16px', background:'#166534', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontWeight:'500' }}>+ Certificación</button>}
          {tab === 'inspecciones' && <button onClick={() => { if(parcelas.length){ setSelParcela(parcelas[0]); setInspeccionModal(true) } else toastError('Crea una parcela primero') }} style={{ fontSize:'12px', padding:'8px 16px', background:'#166534', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontWeight:'500' }}>+ Inspección</button>}
        </div>
      </div>

      <div style={{ display:'flex', gap:'4px', marginBottom:'20px', borderBottom:'0.5px solid #e5e5e3' }}>
        {['dashboard','parcelas','certificaciones','inspecciones'].map(t => <button key={t} onClick={() => setTab(t)} style={{ fontSize:'13px', padding:'8px 16px', background:'none', border:'none', borderBottom: tab===t ? '2px solid #166534' : '2px solid transparent', color: tab===t ? '#166534' : '#9b9b97', cursor:'pointer', fontWeight: tab===t ? '500' : '400', textTransform:'capitalize' }}>{t}</button>)}
      </div>
        ...
        <>
          {tab === 'dashboard' && (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px' }}>
              {[
                { label:'Parcelas', value: parcelas.length, color:'#166534' },
                { label:'Área certificada (ha)', value: totalAreaHa.toFixed(1), color:'#166534' },
                { label:'Certs vigentes', value: certsVigentes, color:'#166534' },
                { label:'Certs vencidas', value: certsVencidas, color:'#dc2626' },
              ].map(k => (
              ...
              {parcelas.length === 0 ? <div style={{ padding:'40px', textAlign:'center', color:'#9b9b97', fontSize:'13px' }}>No hay parcelas registradas</div> : (
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead><tr style={{ background:'#f9f9f7', borderBottom:'0.5px solid #e5e5e3' }}>
                    {['Parcela','Cultivo','Área (ha)','Año conversión','Estado'].map(h => <th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:'11px', fontWeight:'500', color:'#9b9b97' }}>{h}</th>)}
                  </tr></thead>
                  <tbody>{parcelas.map((p:any) => (
                    <tr key={p.id} style={{ borderBottom:'0.5px solid #f0f0ee' }}>
                      ...
                      <td style={{ padding:'10px 14px', fontSize:'12px', color:'#166534', fontWeight:'500', textTransform:'capitalize' }}>{p.cultivo}</td>
                      <td style={{ padding:'10px 14px', fontSize:'13px', fontFamily:'monospace', color:'#166534' }}>{p.areaHa}</td>
                      <td style={{ padding:'10px 14px', fontSize:'12px', color:'#6b6b67' }}>{p.anoConversion||'—'}</td>
                      <td style={{ padding:'10px 14px' }}><span style={{ fontSize:'11px', padding:'3px 8px', borderRadius:'4px', background: p.status==='certificado'?'#dcfce7':p.status==='suspendido'?'#fee2e2':'#fef9c3', color: p.status==='certificado'?'#166534':p.status==='suspendido'?'#dc2626':'#b45309' }}>{p.status}</span></td>
                    </tr>
                  ))}</tbody>
                </table>
              )}
          ...
          {tab === 'certificaciones' && (
            <div style={{ background:'#fff', border:'0.5px solid #e5e5e3', borderRadius:'10px', overflow:'hidden' }}>
              {certs.length === 0 ? <div style={{ padding:'40px', textAlign:'center', color:'#9b9b97', fontSize:'13px' }}>No hay certificaciones registradas</div> : (
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead><tr style={{ background:'#f9f9f7', borderBottom:'0.5px solid #e5e5e3' }}>
                    {['Parcela','Entidad','Tipo','Emisión','Vencimiento','Número','Estado'].map(h => <th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:'11px', fontWeight:'500', color:'#9b9b97' }}>{h}</th>)}
                  </tr></thead>
                  <tbody>{certs.map((c:any) => (
                    <tr key={c.id} style={{ borderBottom:'0.5px solid #f0f0ee' }}>
                      ...
                      <td style={{ padding:'10px 14px', fontSize:'12px', color:'#166534', fontWeight:'500' }}>{c.entidad}</td>
                      <td style={{ padding:'10px 14px', fontSize:'12px', color:'#6b6b67', textTransform:'capitalize' }}>{c.tipo}</td>
                      ...
                      <td style={{ padding:'10px 14px' }}><span style={{ fontSize:'11px', padding:'3px 8px', borderRadius:'4px', background: c.status==='vigente'?'#dcfce7':c.status==='vencida'?'#fee2e2':'#fef9c3', color: c.status==='vigente'?'#166534':c.status==='vencida'?'#dc2626':'#b45309' }}>{c.status}</span></td>
                    </tr>
                  ))}</tbody>
                </table>
              )}
          ...
          {tab === 'inspecciones' && (
            <div style={{ background:'#fff', border:'0.5px solid #e5e5e3', borderRadius:'10px', overflow:'hidden' }}>
              {inspecciones.length === 0 ? <div style={{ padding:'40px', textAlign:'center', color:'#9b9b97', fontSize:'13px' }}>No hay inspecciones registradas</div> : (
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead><tr style={{ background:'#f9f9f7', borderBottom:'0.5px solid #e5e5e3' }}>
                    {['Fecha','Parcela','Inspector','Entidad','Resultado','Próxima'].map(h => <th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:'11px', fontWeight:'500', color:'#9b9b97' }}>{h}</th>)}
                  </tr></thead>
                  <tbody>{inspecciones.map((i:any) => (
                    <tr key={i.id} style={{ borderBottom:'0.5px solid #f0f0ee' }}>
                      ...
                      <td style={{ padding:'10px 14px' }}><span style={{ fontSize:'11px', padding:'3px 8px', borderRadius:'4px', background: i.resultado==='aprobado'?'#dcfce7':i.resultado==='rechazado'?'#fee2e2':'#fef9c3', color: i.resultado==='aprobado'?'#166534':i.resultado==='rechazado'?'#dc2626':'#b45309' }}>{i.resultado||'—'}</span></td>
          ...
          <div style={{ background:'#fff', borderRadius:'12px', padding:'24px', width:'440px', boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}>
            ...
            <form onSubmit={saveParcela}>
              <div style={{ display:'flex', flexDirection:'column', gap:'12px', marginBottom:'20px' }}>
                <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>NOMBRE</label><input value={parcelaForm.nombre} onChange={e=>setParcelaForm({...parcelaForm,nombre:e.target.value})} required style={inp} placeholder="Parcela La Verde"/></div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>CULTIVO</label><input value={parcelaForm.cultivo} onChange={e=>setParcelaForm({...parcelaForm,cultivo:e.target.value})} required style={inp} placeholder="café, cacao, plátano..."/></div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  ...
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>ESTADO</label>
                    <select value={parcelaForm.status} onChange={e=>setParcelaForm({...parcelaForm,status:e.target.value})} style={{...inp,cursor:'pointer'}}>
                      {['en_conversion','certificado','suspendido'].map(s=><option key={s} value={s}>{s}</option>)}</select></div>
                </div>
              </div>
              <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end' }}>
                ...
                <button type="submit" style={{ fontSize:'12px', padding:'8px 16px', background:'#166534', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontWeight:'500' }}>Crear parcela</button>
              </div>
            </form>
          </div>
        </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>ENTIDAD</label>
                    <select value={certForm.entidad} onChange={e=>setCertForm({...certForm,entidad:e.target.value})} style={{...inp,cursor:'pointer'}}>
                      {ENTIDADES_CERT.map(e=><option key={e}>{e}</option>)}</select></div>
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>TIPO</label>
                    <select value={certForm.tipo} onChange={e=>setCertForm({...certForm,tipo:e.target.value})} style={{...inp,cursor:'pointer'}}>
                      {TIPOS_CERT.map(t=><option key={t}>{t}</option>)}</select></div>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  ...
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>VENCIMIENTO</label><input type="date" value={certForm.fechaVencimiento} onChange={e=>setCertForm({...certForm,fechaVencimiento:e.target.value})} style={inp}/></div>
                </div>
                <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>NÚMERO DE CERTIFICADO</label><input value={certForm.numeroDoc} onChange={e=>setCertForm({...certForm,numeroDoc:e.target.value})} style={inp} placeholder="Opcional"/></div>
              </div>
              <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end' }}>
                ...
                <button type="submit" style={{ fontSize:'12px', padding:'8px 16px', background:'#166534', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontWeight:'500' }}>Guardar certificación</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {inspeccionModal && selParcela && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.3)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50 }}>
          <div style={{ background:'#fff', borderRadius:'12px', padding:'24px', width:'440px', boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ fontSize:'16px', fontWeight:'500', marginBottom:'6px' }}>🔍 Registrar inspección</div>
            <div style={{ fontSize:'12px', color:'#9b9b97', marginBottom:'20px' }}>{selParcela.nombre}</div>
            <form onSubmit={saveInspeccion}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  ...
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>RESULTADO</label>
                    <select value={inspeccionForm.resultado} onChange={e=>setInspeccionForm({...inspeccionForm,resultado:e.target.value})} style={{...inp,cursor:'pointer'}}>
                      {['aprobado','observaciones','rechazado'].map(r=><option key={r}>{r}</option>)}</select></div>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  <div><label style={{ display:'block', fontSize:'11px', color:'#9b9b97', marginBottom:'5px', fontWeight:'500' }}>INSPECTOR</label><input value={inspeccionForm.inspector} onChange={e=>setInspeccionForm({...inspeccionForm,inspector:e.target.value})} style={inp} placeholder="Nombre del inspector"/></div>
              ...
              <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end' }}>
                ...
                <button type="submit" style={{ fontSize:'12px', padding:'8px 16px', background:'#166534', color:'white', border:'none', borderRadius:'6px', cursor:'pointer', fontWeight:'500' }}>Guardar inspección</button>
              </div>
            </form>
          </div>
        </div>
